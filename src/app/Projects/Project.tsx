	...
	return (
		<section className='py-24 px-4  text-gray-700 dark:text-white transition-colors duration-500'>
			<div className='max-w-7xl mx-auto'>
				...
				<div className='grid xl:gap-x-20 xl:gap-y-28 lg:gap-y-12 lg:gap-x-10 gap-y-16  sm:grid-cols-1 lg:grid-cols-2'>
					{myProjects.slice(0, 4).map((project, index) => (
						<motion.div
							...
							className='md:rounded-[30px]  rounded-xl overflow-hidden group px-2 lg:px-3 xl:px-6 bg-white dark:bg-darkprimary dark:border-darkthird/25 border-2 shadow-xl hover:shadow-sm transition duration-300'>
							<div className='relative w-full h-[250px] md:h-[400px] overflow-hidden bg-gray-100 dark:bg-gray-950 dark:border-b-2 border-orange-900/60 '>
								<Image
									src={project.image}
									alt='Tech tag'
									fill
				...
				<Link
					href='/Projects'
					className=' w-44 rounded-full bg-yellow-500  text-white  font-semibold text-center py-3 mt-16 block mx-auto hover:bg-yellow-600 dark:hover:bg-yellow-500 transition-colors duration-300 text-xl'>
					{t('AllProjects')}
				</Link>
			</div>
		</section>
